...
namespace test
{
	...
	public class DebugString  : IDisposable
	{
		...
		{
			return new Texture2D(resourceName, false);
		}
		...
		public DebugString(GraphicsContext graphics, Texture2D texture, int charaWidth, int charaHeight)
		{
			...
			if( texture != null)
				...
				this.texture = CreateTextureFromEmbeddedFile("/Application/resources/DebugFont.png");
...
//			shaderProgram.SetUniformValue(0, ref unitScreenMatrix);
		...
		{
			ShaderProgram shaderProgram = new ShaderProgram("/Application/shaders/SimpleSprite.cgx");
